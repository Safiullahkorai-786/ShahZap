-- Add continent-based matching to match_preferences.
-- Continents map to ISO country codes server-side in match_next.

alter table public.match_preferences
  add column if not exists preferred_continents text[] not null default '{}';

-- Map continent key (lowercase_underscore) → set of ISO-2 country codes.
...
-- Expose to client for UI.
grant execute on function public.continent_countries(text) to authenticated, anon;
