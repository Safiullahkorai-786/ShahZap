...
begin
  ...
  -- Expire other users, but keep the requester waiting so the configured timeout
  -- can transition from interest-priority matching to safe random fallback.
  update public.match_queue set status='expired'
  where status='waiting' and expires_at <= now() and profile_id <> p_profile_id;
