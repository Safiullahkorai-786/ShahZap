-- Updated match_next: adds interest filter + shared-interest scoring boost.
-- preferred_interests uses interest SLUGS (interests.slug).
-- If my preferred_interests is non-empty, candidates must share at least one.
-- Shared interests boost ordering so interested people meet each other first.
...
begin
  ...
  for candidate_id, candidate_expires in
    ...
    where q.status='waiting' and q.profile_id<>p_profile_id and cp.profile_visible=true
      ...
      and (coalesce(cardinality(my_prefs.preferred_interests),0)=0 or exists(
            select 1 from public.profile_interests pi
            join public.interests i on i.id=pi.interest_id
            where pi.profile_id=q.profile_id and i.slug=any(my_prefs.preferred_interests)))
      ...
      and not exists(select 1 from public.match_preferences op where op.profile_id=q.profile_id and cardinality(op.preferred_interests)>0 and not exists(
            select 1 from public.profile_interests pi2
            join public.interests i2 on i2.id=pi2.interest_id
            where pi2.profile_id=p_profile_id and i2.slug=any(op.preferred_interests)))
    order by
      case when exists(select 1 from public.match_queue mq where mq.profile_id=p_profile_id and mq.status='waiting' and mq.expires_at>now()) then 1 else 0 end * (
        -- Interest overlap score (0-100): shared interests boost heavily
        coalesce((
          select count(*) * 20 from public.profile_interests pi3
          join public.interests i3 on i3.id=pi3.interest_id
          where pi3.profile_id=q.profile_id
            and (coalesce(cardinality(my_prefs.preferred_interests),0)=0 or i3.slug=any(my_prefs.preferred_interests))
        ), 0)
        + coalesce(public.matching_score(p_profile_id, q.profile_id), 0)
      ) desc,
      q.queued_at asc
    limit 20
  loop
