-- FIX: when ADDING a reaction, the "strip me from other emojis" query
-- overwrites next_reactions with a filtered copy that excludes the target key.
-- Then reading next_reactions -> p_emoji returns NULL, losing all existing
-- users who already reacted with that same emoji.
--
-- Fix: read the target emoji's array BEFORE the filtering step.
...
begin
  ...
  if already_had then
    ...
    -- 1. Preserve the target emoji's existing array FIRST.
    -- 2. Strip me from OTHER emojis only (one-reaction-per-person).
